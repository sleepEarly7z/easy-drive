...
const TopRecommended = () => {
  const NextArrow = ({ onClick }) => {
    return (
      <div className="arrow_topRecommended next_topRecommended" onClick={onClick}>
        <FaArrowRight />
      </div>
    );
  };
  ...
  const PrevArrow = ({ onClick }) => {
    return (
      <div className="arrow_topRecommended prev_topRecommended" onClick={onClick}>
        <FaArrowLeft />
      </div>
    );
  };
  ...
  return (
    <div className="TopRecommended">
      <h1>Top Recommended Instructors</h1>
      <Slider {...settings}>
        {images.map((img, idx) => (
          <div className={idx === imageIndex ? "slide_topRecommended activeSlide" : "slide_topRecommended"}>
            {/* <img src={img} alt={img} /> */}
      ...
      <div className="explore_more_div">
            <button className="explore_more">Explore</button>
      </div>
    </div>
  );
}
